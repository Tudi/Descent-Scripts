...
class HeadlessHorsemanFireAI : public MoonScriptCreatureAI
{
	...
	{}
	int32	FireVisualSpell;
	uint32	ReigniteStamp;
	void OnLoad()
	{
		FireVisualSpell = 2;
		_unit->CastSpell( _unit, 42075, true );
		...
		_unit->SetUInt32Value( UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NOT_ATTACKABLE_2 | UNIT_FLAG_NOT_ATTACKABLE_9 );
	}
	void SetValue(int ValueName,int32 Value)
	{
		//bucket of water landed on us
		if( Value == 42339 )
		{
			//water sprinkle visual
			_unit->CastSpell( _unit, 42348, false );
			if( FireVisualSpell <= 0 )
				_unit->Despawn( 500, 0 );
			else
			{
//				ReigniteStamp = GetTickCount() + 30000;
				ReigniteStamp = GetTickCount() + 10000;
				uint32 FireSpells[3] = { 42971, 42074, 42075 };
				_unit->RemoveAura( FireSpells[ FireVisualSpell ], 0, AURA_SEARCH_POSITIVE );
				FireVisualSpell--;
				_unit->CastSpell( _unit, FireSpells[ FireVisualSpell ], true );
			}
		}
		//script is lighting the fire. We don't actually have this
		else if( Value == 42132 )
		{
			_unit->CastSpell( _unit, 42075, true );
		}
	...
	void AIUpdate()
	{
		if( ReigniteStamp && ReigniteStamp < GetTickCount() )
		{
			if( FireVisualSpell < 1 )
//				ReigniteStamp = GetTickCount() + 30000;
				ReigniteStamp = GetTickCount() + 10000;
			else
				ReigniteStamp = 0;
			if( FireVisualSpell < 2 )
			{
				uint32 FireSpells[3] = { 42971, 42074, 42075 };
				_unit->RemoveAura( FireSpells[ FireVisualSpell ], 0, AURA_SEARCH_POSITIVE );
				FireVisualSpell++;
...
class ShadeOfTheHorsemanAI : public MoonScriptCreatureAI
{
	...
	ShadeOfTheHorsemanAI(Creature* pCreature) : MoonScriptCreatureAI(pCreature)
	{
		...
		{
			WayPoint *w = CreateWaypoint( i, 0, WayPoints[i].mAddition, WayPoints[i]);
			AddWaypoint( w );
		}
	}
