...
RegType<GameObject> GOMethods[] = {
	...
	{ "SetByte", &luaGameObject_SetByteValue },
	...
	{ "RegisterEvent", &luaGameObject_RegisterEvent },
...
void LuaEngine::CallFunction(GameObject * pGo, const char * FuncName)
{
	m_Lock.Acquire();
	lua_pushstring(L, FuncName);
	lua_gettable(L, LUA_GLOBALSINDEX);
	if(lua_isnil(L,-1))
	{
		printf("Tried to call invalid LUA function '%s' from Ascent (GameObject)!\n", FuncName);
		m_Lock.Release();
		return;
	}
...
public:
	...
	void StringFunctionCallStr(const char * pFunction)
	{
		g_engine->CallFunction( _gameobject, pFunction );
	}	
	void AIUpdate()
	{
...
int luaUnit_GetClosestPlayer(lua_State * L, Unit * ptr)
{
	...
	for(InRangePlayerSetRecProt::iterator itr = ptr->GetInRangePlayerSetBegin( AutoLock ); itr != ptr->GetInRangePlayerSetEnd(); ++itr)
	{
...
int luaUnit_GetRandomFriend(lua_State * L, Unit * ptr)
{
	...
	InrangeLoopExitAutoCallback AutoLock;
	for(InRangeSetRecProt::iterator itr = ptr->GetInRangeSetBegin( AutoLock ); itr != ptr->GetInRangeSetEnd(); ++itr)
	{
...
int luaUnit_ReturnToSpawnPoint(lua_State * L, Unit * ptr)
{
	if( ptr->IsCreature() )
	{
		float x = SafeCreatureCast( ptr )->GetSpawnX();
		float y = SafeCreatureCast( ptr )->GetSpawnY();
		float z = SafeCreatureCast( ptr )->GetSpawnZ();
		float o = SafeCreatureCast( ptr )->GetSpawnO();
		if(ptr)
...
int luaUnit_CastSpellAoF(lua_State * L, Unit * ptr)
{
    ...
    if(!sp | !x | !y | !z)
        return 0;
	ptr->CastSpellAoF( NULL, x,y,z,dbcSpell.LookupEntry(sp), true);
	return 0;
}
...
//	lua_pushinteger(L,(ptr->GetZoneId()));
	lua_pushinteger(L,GetProbableZoneIdForLocation( ptr->GetMapId(), ptr->GetPositionX(), ptr->GetPositionY(), false ) );
	return 1;
}
...
int luaUnit_GetSpawnX(lua_State * L, Unit * ptr)
{
	...
	{
		if( ptr->IsCreature() )
			lua_pushnumber(L,SafeCreatureCast( ptr )->GetSpawnX());
		else
			lua_pushnumber(L, ptr->GetPositionX());
		return 1;
	}
...
int luaUnit_GetSpawnY(lua_State * L, Unit * ptr)
{
	...
	{
		if( ptr->IsCreature() )
			lua_pushnumber(L,SafeCreatureCast( ptr )->GetSpawnY());
		else
			lua_pushnumber(L, ptr->GetPositionY());
		return 1;
	}
...
int luaUnit_GetSpawnZ(lua_State * L, Unit * ptr)
{
	...
	{
		if( ptr->IsCreature() )
			lua_pushnumber(L,SafeCreatureCast( ptr )->GetSpawnZ());
		else
			lua_pushnumber(L, ptr->GetPositionZ());
		return 1;
	}
...
int luaUnit_GetSpawnO(lua_State * L, Unit * ptr)
{
	...
	{
		if( ptr->IsCreature() )
			lua_pushnumber(L,SafeCreatureCast( ptr )->GetSpawnO());
		else
			lua_pushnumber(L, ptr->GetOrientation());
		return 1;
	}
...
int luaUnit_Energize(lua_State * L, Unit * ptr)
{
	...
	if(!target||!spellid||!amount||!type||!ptr)
		return 0;
	ptr->Energize(target,spellid,amount,type,0);
	return 0;
}
...
int luaUnit_Kill(lua_State * L, Unit * ptr)
{
	...
	ptr->DealDamage(target,target->GetUInt32Value(UNIT_FIELD_HEALTH),0,true);
	return 0;
}
int luaUnit_DealDamage(lua_State * L, Unit * ptr)
{
	...
	if(!ptr||!target)
		return 0;
	ptr->DealDamage(target,damage,spellid,true);
	return 0;
}
...
int luaUnit_IsInvisible(lua_State * L, Unit * ptr)
{
	...
	if(enabled > 0)
	{
		ptr->m_invisFlag |= INVIS_FLAG_CUSTOM_SCRIPTS;
	}
	else
	{
		ptr->m_invisFlag &= ~INVIS_FLAG_CUSTOM_SCRIPTS;
...
int luaUnit_SpellNonMeleeDamageLog(lua_State * L, Unit * ptr)
{
	...
	if(pVictim&&spellid&&damage&&allowproc&&static_dmg&&no_remove_auras)
	{
		...
		tdmg.StaticDamage = ((static_dmg>0)?true:false);
		tdmg.DisableProc = !((allowproc >0)?true:false);
//		ptr->SpellNonMeleeDamageLog(pVictim,spellid,&tdmg,((no_remove_auras>0)?true:false));
		ptr->SpellNonMeleeDamageLog( pVictim, dbcSpell.LookupEntryForced( spellid ), &tdmg );
	}
	return 0;
}
...
int luaUnit_EventCastSpell(lua_State * L, Unit * ptr)
{
	...
	if ( ptr && sp)
	{
		...
		{
		case TYPEID_PLAYER:
			sEventMgr.AddEvent(ptr, &Player::EventCastSpell2,target->GetGUID(),sp,true,EVENT_PLAYER_UPDATE,delay,repeats,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		case TYPEID_UNIT:
			sEventMgr.AddEvent(ptr, &Unit::EventCastSpell2,target->GetGUID(),sp,true,EVENT_CREATURE_UPDATE,delay,repeats,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
	}
	return 0;
}
...
int luaGameObject_GetSpawnX(lua_State * L, GameObject * ptr)
{
	...
	{
		if( ptr->m_spawn )
			lua_pushnumber(L,ptr->m_spawn->x);
		else
			lua_pushnumber(L,ptr->GetPositionX());
	}
	return 1;
...
int luaGameObject_GetSpawnY(lua_State * L, GameObject * ptr)
{
	...
	{
		if( ptr->m_spawn )
			lua_pushnumber(L,ptr->m_spawn->y);
		else
			lua_pushnumber(L,ptr->GetPositionY());
	}
	return 1;
...
int luaGameObject_GetSpawnZ(lua_State * L, GameObject * ptr)
{
	...
	{
		if( ptr->m_spawn )
			lua_pushnumber(L,ptr->m_spawn->z);
		else
			lua_pushnumber(L,ptr->GetPositionZ());
	}
	return 1;
...
int luaGameObject_GetSpawnO(lua_State * L, GameObject * ptr)
{
	...
	{
		if( ptr->m_spawn )
			lua_pushnumber(L,ptr->m_spawn->o);
		else
			lua_pushnumber(L,ptr->GetOrientation());
	}
	return 1;
...
int luaGameObject_SetByteValue(lua_State * L, GameObject * ptr)
{
	unsigned int field = luaL_checkint(L,1);
	unsigned int byte = luaL_checkint(L,2);
	int value = luaL_checkint(L,3);
	if(ptr && field && byte < 4 && value )
	{
		ptr->SetByte(field,byte,value);
	}
	return 1;
}
...
int luaGameObject_CastSpell(lua_State * L, GameObject * ptr)
{
	...
	Spell * spp = SpellPool.PooledNew( __FILE__, __LINE__ );
...
int luaGameObject_RegisterEvent(lua_State * L, GameObject * ptr)
{
	if( !ptr || ptr->IsGameObject() == false )
		return 0;
	...
	GameObject * pGo = ((GameObject*)ptr);
	string strFunc = string(func_to_call);
	sEventMgr.AddEvent(pGo, &GameObject::TriggerScriptEventStr, strFunc, EVENT_GAMEOBJECT_UPDATE, (uint32)delay, (uint32)repeats, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	return 0;
}
