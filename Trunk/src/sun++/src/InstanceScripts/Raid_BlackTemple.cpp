...
class MutantWarHoundAI : public MoonScriptCreatureAI
{
	...
	void OnDied(Unit *pKiller)
	{
		...
		Aura *pAura = AuraPool.PooledNew( __FILE__, __LINE__ );
		if (pAura != NULL)
		{
				...
				if(m_spellcheck[i])
				{
					...
					switch(spells[i].targettype)
					{
						...
						case TARGET_DESTINATION:
							_unit->CastSpellAoF(target,target->GetPositionX(),target->GetPositionY(),target->GetPositionZ(), spells[i].info, spells[i].instant); break;
	...
	void AIUpdate()
	{
		...
		if (Phase == 2 && _unit->GetAIInterface()->GetNextTarget() && _unit->GetCurrentSpell() == NULL)
		{
			...
			if (t > spells[7].casttime - 62 && LastThreat != 0)
			{
				...
				if (RageTarget->HasAura(FEL_RAGE1) && RageTarget->HasAura(FEL_RAGE2))
				{
			...
			if (LastThreat != 0 && RandomUInt(5) == 1 && t > spells[6].casttime)
			{
				...
				if (RageTarget->HasAura(FEL_RAGE1) || RageTarget->HasAura(FEL_RAGE2))
				{
					...
					_unit->CastSpellAoF(RageTarget,RageTarget->GetPositionX(),RageTarget->GetPositionY(),RageTarget->GetPositionZ(), spells[6].info, spells[6].instant);
...
public:
	...
	void AIUpdate()
	{
		...
		if (_unit->GetAIInterface()->GetNextTarget() && _unit->GetCurrentSpell() == NULL && !_unit->HasAura(VANISH))
		{
			uint32 t = (uint32)time(NULL);
			if (t > spells[2].casttime)
			{
			...
			if (_unit->GetAIInterface()->GetNextTarget() && t > spells[1].casttime && Poisoned && RandomUInt(3) != 1)
			{
				...
				{ 
					if (isHostile(_unit, (*itr)) && ((*itr)->GetTypeId()== TYPEID_UNIT || (*itr)->GetTypeId() == TYPEID_PLAYER) && (*itr)->GetInstanceID() == _unit->GetInstanceID())
					{
						...
						if (PoisonedTarget->isAlive() && _unit->GetAIInterface()->getThreatByPtr(PoisonedTarget) > 0 && PoisonedTarget->HasAura(DEADLY_POISON))
						{
	...
	void SpellCast(float val)
    {
        if(_unit->GetCurrentSpell() == NULL && _unit->GetAIInterface()->GetNextTarget() && !_unit->HasAura(VANISH))
        {
			float comulativeperc = 0;
		    Unit *target = NULL;
			for(int i=0;i<nrspells;i++)
...
class AkamaAI : public MoonScriptBossAI
{
    ...
    void OnReachWP(uint32 iWaypointId, bool bForwards)
    {
		...
		case 17:
			...
			break;
		case 19:
			_unit->m_invisFlag |= INVIS_FLAG_CUSTOM_SCRIPTS;						// Arc's
	...
	Unit *FindClosestTargetToUnit(Unit *pSeeker)
	{
		...
		InrangeLoopExitAutoCallback AutoLock;
		for (InRangePlayerSetRecProt::iterator itr = _unit->GetInRangePlayerSetBegin( AutoLock ); itr != _unit->GetInRangePlayerSetEnd(); ++itr)
		{
			pUnit = static_cast<Unit*>(*itr);
		...
		if ((mIllidanAI != NULL && mScenePart <= 6) || mScenePart > 6)
		{
			...
			case 1:
				...
				if (mIllidanAI->GetUnit()->GetCurrentSpell() != NULL)
					mIllidanAI->GetUnit()->GetCurrentSpell()->safe_cancel();
				break;
			case 2:
			...
			case 6:{
				...
				if (pAkama != NULL)
				{
					...
					pAkama->m_invisFlag |= INVIS_FLAG_CUSTOM_SCRIPTS;						// Arc's
	...
	void PhaseTwo()
	{
		if (mScenePart != 0)
		{
			...
			case 1:
				for (int i = 0; i < 2; ++i)
				{
					_unit->CastSpellAoF(NULL,UnitPos[i].mX, UnitPos[i].mY, UnitPos[i].mZ, dbcSpell.LookupEntry(ILLIDAN_THROW_GLAIVE1), false);
				}
				SetWieldWeapon(false);
				break;
			case 2:
