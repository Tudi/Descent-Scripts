	...
	void AIUpdate()
    {
		...
		case 1:
				...
				PhaseOneTimer == 60 || PhaseOneTimer == 45 || PhaseOneTimer == 30 || PhaseOneTimer == 15)
			{
				...
				if (Magtheridon)
				{
					Aura *aura = Magtheridon->HasAura(BANISH);
					if (aura)
					{
						EventStarted = false;
						PhaseOneTimer = 0;
...
public:
	...
	void OnActivate(Player * pPlayer)
	{
		...
		Aura *aura = pPlayer->HasAura(MIND_EXHAUSTION);
		if (aura)
			return;
	...
	void AIUpdate()
	{
		...
		Aura* aura = NULL;
		if (Channeler)
			aura = Channeler->HasAura(SHADOW_GRASP2);
		...
		if (Counter >= ACTIVE_CUBES_TO_BANISH && Magtheridon && Magtheridon->isAlive())
		{
			...
			if(Magtheridon->GetCurrentSpell())
				Magtheridon->GetCurrentSpell()->safe_cancel();
...
public:
	...
	void PhaseTwo()
	{
		Aura *aura = _unit->HasAura(BANISH);
		...
		if(timer_caveIn && (timer_caveIn != 1 || (_unit->GetCurrentSpell() == NULL && timer_caveIn == 1 && !aura)))
		{
			...
			if (timer_caveIn == 5)
			{
				for (int i = 0; i < 6; i++)
				{
					...
					if (Trigger)
					{
						Trigger->CastSpellAoF(NULL,CaveInPos[i].x, CaveInPos[i].y, CaveInPos[i].z, spells[5].info, spells[5].instant);
					}
				}
