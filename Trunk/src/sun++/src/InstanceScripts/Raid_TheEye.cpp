...
bool Dummy_Solarian_WrathOfTheAstromancer(uint32 pEffectIndex, Spell* pSpell)
{
	...
	sEventMgr.AddEvent(Target, &Unit::EventCastSpell2, Target->GetGUID(), SpellInfo->Id, true, EVENT_UNK, 6000, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	return true;
}
...
//			AlarAuxClass *cAux=new AlarAuxClass(this);
	...
	{
		InrangeLoopExitAutoCallback AutoLock;
		for(InRangeSetRecProt::iterator i = _unit->GetInRangeSetBegin( AutoLock ); i != _unit->GetInRangeSetEnd(); ++i)
		{
    ...
    void PhaseOne()
    {
		if (FlameQuills == true)
		{
			...
			{
				_unit->CastSpellAoF(NULL,fly[1].mX,fly[1].mY,fly[1].mZ, spells[1].info, spells[1].instant); 
				_unit->CastSpellAoF(NULL,fly[2].mX,fly[2].mY,fly[2].mZ, spells[1].info, spells[1].instant); 
				_unit->CastSpellAoF(NULL,fly[3].mX,fly[3].mY,fly[3].mZ, spells[1].info, spells[1].instant); 
				_unit->CastSpellAoF(NULL,fly[4].mX,fly[4].mY,fly[4].mZ, spells[1].info, spells[1].instant); 
				_unit->GetAIInterface()->m_canMove = true;
				_unit->GetAIInterface()->setMoveType(MOVEMENTTYPE_WANTEDWP);
	...
	void CastSpell(int id)
	{
		...
		switch(spells[id].targettype)
		{
			...
			case TARGET_DESTINATION:
				_unit->CastSpellAoF(target,target->GetPositionX(),target->GetPositionY(),target->GetPositionZ(), spells[id].info, spells[id].instant); break;
			case TARGET_RANDOM_SINGLE: {
...
public:
    ...
    void OnDied(Unit * mKiller)
    {
		...
		if (Phase==2/*&& Alar->isAlive()*/) {
			...
			Alar->DealDamage(Alar,(maxhp*3)/100,0);
		}
    }
