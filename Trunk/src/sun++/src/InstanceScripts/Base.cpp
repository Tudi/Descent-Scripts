...
Unit* MoonScriptCreatureAI::ForceCreatureFind(uint32 pCreatureId, float pX, float pY, float pZ, bool pCurrentPosition)
{
	...
	if ( pMapMgr == NULL || pMapMgr->GetInterface() == NULL ) //shutting down ?
		return pUnit;
...
void MoonScriptCreatureAI::RemoveAuraOnPlayers(uint32 pSpellId)
{
	...
	for( InRangePlayerSetRecProt::iterator PlayerIter = _unit->GetInRangePlayerSetBegin( AutoLock ); PlayerIter != _unit->GetInRangePlayerSetEnd(); ++PlayerIter ) 
	{
...
void MoonScriptCreatureAI::CastSpellOnTarget(Unit* pTarget, TargetType pType, SpellEntry* pEntry, bool pInstant)
{
	switch( pType )
	{
		...
		case Target_RandomPlayerDestination:
			_unit->CastSpellAoF( pTarget, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), pEntry, pInstant);
			break;
...
Unit* MoonScriptCreatureAI::GetBestUnitTarget(TargetFilter pFilter)
{
	...
	{
		InrangeLoopExitAutoCallback AutoLock;
		for( InRangeSetRecProt::iterator ObjectIter = _unit->GetInRangeSetBegin( AutoLock ); ObjectIter != _unit->GetInRangeSetEnd(); ++ObjectIter )
		{
	...
	{
//		for( InRangeSet::iterator ObjectIter = _unit->GetInRangeOppFactsSetBegin(); ObjectIter != _unit->GetInRangeOppFactsSetEnd(); ++ObjectIter )
		InrangeLoopExitAutoCallback AutoLock;
...
bool MoonScriptCreatureAI::IsValidUnitTarget(Object* pObject, TargetFilter pFilter)
{
	...
	if( UnitTarget->IsInvisible() ) return false;
