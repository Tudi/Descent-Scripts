					...
					case TARGET_RANDOM_SINGLE:
						{
							...
							for(InRangePlayerSetRecProt::iterator itr = _unit->GetInRangePlayerSetBegin( AutoLock ); 
...
std::map<uint32,uint32> WayStartBBW;
...
public:
	...
	void FlameWreath()
	{
		...
		InrangeLoopExitAutoCallback AutoLock;
		InRangePlayerSetRecProt::iterator hostileItr = _unit->GetInRangePlayerSetBegin( AutoLock );
...
public:
	...
	void SummonInfernal()
	{
		...
		InfernalDummy->CastSpellAoF(NULL,ranX, ranY, 275.0f, spells[2].info, spells[2].instant); // Shoots the missile
	...
	void FlyPhase()
	{
		...
		if(!m_FlyPhaseTimer)
		{
			if(_unit->GetCurrentSpell() != NULL)
				_unit->GetCurrentSpell()->safe_cancel();
		...
		if(m_FlyPhaseTimer == 15)
		{
			...
			//summon 3 skeletons
			//_unit->CastSpellAoF(target,target->GetPositionX(),target->GetPositionY(),target->GetPositionZ(), dbcSpell.LookupEntry(SUMMON_BONE_SKELETONS), true);
			return;
		}
		...
		for(InRangeSetRecProt::iterator itr = _unit->GetInRangeSetBegin( AutoLock ); itr != _unit->GetInRangeSetEnd(); ++itr) 
		{
			if ((*itr)->GetTypeId() == TYPEID_PLAYER && (*itr)->GetInstanceID() == _unit->GetInstanceID())
			{
				...
				{
					_unit->CastSpellAoF(target,target->GetPositionX(),target->GetPositionY(),target->GetPositionZ(), dbcSpell.LookupEntry(FIREBALL_BARRAGE), true);
					break; //stop
				}
			}
		}
...
//		_unit = NULL; //hmmm, can we even do this ?
	...
	void OnDied(Unit * mKiller)
	{
		if( _unit == NULL || _unit->GetMapMgr() == NULL )
			return;
		//Check to see if we can spawn The Crone now
		Creature* Dorothee	= _unit->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(-10897.650f, -1755.8311f, 90.476f, 17535);	//Dorothee
		Creature* Strawman	= _unit->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(-10904.088f, -1754.8988f, 90.476f, 17543);	//Strawman
...
public:
	...
	void OnDied(Unit * mKiller)
	{
		if( _unit == NULL || _unit->GetMapMgr() == NULL || _unit->GetMapMgr()->GetInterface() == NULL )
			return;
		//Check to see if we can spawn The Crone now
		Creature* Dorothee	= _unit->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(-10897.650f, -1755.8311f, 90.476f, 17535);	//Dorothee
		Creature* Strawman	= _unit->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(-10904.088f, -1754.8988f, 90.476f, 17543);	//Strawman
