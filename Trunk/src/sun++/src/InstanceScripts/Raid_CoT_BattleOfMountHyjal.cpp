...
std::map<uint32,uint32> HyjalPhase;
	...
	void MarkCast()
	{
		...
		{ 
			if (isHostile(_unit, (*itr)) && ((*itr)->GetTypeId()== TYPEID_UNIT || (*itr)->GetTypeId() == TYPEID_PLAYER) && (*itr)->GetInstanceID() == _unit->GetInstanceID())
			{
				...
				if (Target->isAlive() && Target->HasAura(MARK_OF_KAZROGAL) && (Target->GetPowerType() != POWER_TYPE_MANA || !Target->GetUInt32Value(UNIT_FIELD_BASE_MANA)))
				{
