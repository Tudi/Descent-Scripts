...
public:
	...
	HydrossTheUnstableAI(Creature* pCreature) : CreatureAIScript(pCreature)
	{
		...
		//frost immunity
		_unit->SchoolImmunityAntiEnemy[SCHOOL_FROST] = 1;
	}
	...
	void OnCombatStop(Unit *mTarget)
	{
		...
		_unit->SchoolImmunityAntiEnemy[SCHOOL_FROST] = 1;
		_unit->SchoolImmunityAntiEnemy[SCHOOL_NATURE] = 0;
	...
	void AIUpdate()
	{
		if(!form) //water form
		{
			...
			if(dist > 25)
			{
				...
				_unit->SchoolImmunityAntiEnemy[SCHOOL_FROST] = 0;
				_unit->SchoolImmunityAntiEnemy[SCHOOL_NATURE] = 1;
				...
				summon = _unit->GetMapMgr()->GetInterface()->SpawnCreature(CN_TAINTED_SPAWN_OF_HYDROSS, 
					...
					false, false, 0, 0);
				if(summon) summon->SchoolImmunityAntiEnemy[SCHOOL_NATURE] = 1;
				summon = _unit->GetMapMgr()->GetInterface()->SpawnCreature(CN_TAINTED_SPAWN_OF_HYDROSS, 
					posx-6.93f, posy+11.25f, posz, orientation,
					false, false, 0, 0);
				if(summon) summon->SchoolImmunityAntiEnemy[SCHOOL_NATURE] = 1;
		...
		else //poison form
		{
			...
			if(dist < 25)
			{
				...
				summon = _unit->GetMapMgr()->GetInterface()->SpawnCreature(CN_PURE_SPAWN_OF_HYDROSS, 
					posx+6.93f, posy-11.25f, posz, orientation,
					false, false, 0, 0);
				if(summon) summon->SchoolImmunityAntiEnemy[SCHOOL_FROST] = 1;
				summon = _unit->GetMapMgr()->GetInterface()->SpawnCreature(CN_PURE_SPAWN_OF_HYDROSS, 
					posx-6.93f, posy+11.25f, posz, orientation,
					false, false, 0, 0);
				if(summon) summon->SchoolImmunityAntiEnemy[SCHOOL_FROST] = 1;
...
std::map<uint32,uint32> LeotherasEventGreyheartToKill;
/*
...
public:
	...
	void FirstCheck()
	{
		...
		for(InRangeSet::iterator itr = _unit->GetInRangeSetBegin( AutoLock ); itr != _unit->GetInRangeSetEnd(); ++itr) 
		{
...
public:
	...
	void AIUpdate()
	{
		...
		if(!CataclysmicBoltTimer)
		{
			...
			{
				info_cataclysmic_bolt->eff[0].EffectBasePoints = target->GetUInt32Value(UNIT_FIELD_MAXHEALTH)/2;
	...
	void PhaseTwo()
	{
		...
		if(!CoilfangStriderTimer)
		{
			...
			if(summoned)
			{
				...
				float nearestdist = 0;
				InrangeLoopExitAutoCallback AutoLock;
				for(InRangeSetRecProt::iterator itr = summoned->GetInRangeSetBegin( AutoLock ); itr != summoned->GetInRangeSetEnd(); ++itr) 
				{
