...
bool Execute(uint32 i, Spell* pSpell)
{
    ...
    */
	...
	uint32 power = pSpell->u_caster->GetUInt32Value(UNIT_FIELD_POWER2);
    uint32 add_damage = (3 * pSpell->GetProto()->RankNumber);

	if( pSpell && pSpell->p_caster && pSpell->HasGlyphWithID( GLYPH_WARRIOR_EXECUTE ) )
		power += 100;

    add_damage *=  power / 10;   // rage is *10 always
