    ...
    void GossipSelectOption(Object * pObject, Player * plr, uint32 Id, uint32 IntId, const char * Code)
    {
		...
		switch (IntId)	// switch and case 0 can be deleted, but I added it, because in future maybe we will have to expand script with more options.
		{
			...
			case 1:
			{
				...
				else
				{
					...
					else
					{
						...
						int32 gold = plr->GetGold();
						plr->SetGold( gold - 600);
					}	
				}
			}break;
			...
			case 2:
			{
				...
				else if ((plr->GetGold() < 250000 && plr->getLevel() <= 50) ||
						(plr->GetGold() < 500000 && plr->getLevel() > 50 && plr->getLevel() <= 65) ||
						(plr->GetGold() < 1000000 && plr->getLevel() > 65))
				{
					SendQuickMenu(20008);
				}
				...
				else
				{
                    ...
                    plr->SetGold( plr->GetGold() - unlearnGold);
