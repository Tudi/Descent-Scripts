...
protected:
    uint32 m_readPos;
    uint32 m_writePos;
	uint8 * m_bufferPointer;		//this is a pointer on the stack
	uint8 * m_heapBuffer;
	uint32 m_space;
...
public:
	 ...
	 */
	StackBuffer(uint8 *stackp, uint32 stack_size ) : m_readPos(0), m_writePos(0), m_bufferPointer(stackp), m_heapBuffer(NULL), m_space(stack_size) {}
	...
	void ReallocateOnHeap()
	{
		...
		m_heapBuffer = (uint8*)realloc(m_heapBuffer, m_space + 200);
		m_space += 200;
	 ...
	 */
	ARCEMU_INLINE uint32 GetBufferSize() { return m_writePos; }
...
#define DEFINE_FAST_WRITE_OPERATOR(type, size) StackBuffer& operator << (type src) { if(m_writePos + size > m_space) { ReallocateOnHeap(); } *(type*)&m_bufferPointer[m_writePos] = src; m_writePos += size; return *this; }
	 ...
	 */
	StackBuffer& operator << (std::string & value) { EnsureBufferSize((uint32)value.length() + 1); memcpy(&m_bufferPointer[m_writePos], value.c_str(), (uint32)value.length()+1); m_writePos += ((uint32)value.length() + 1); return *this; }
	...
	StackBuffer& operator >> (WoWGuid & value)
	{
		...
		value.Init(mask);
		for(int32 i = 0; i < BitCount8(mask); ++i)
	...
	/** Clears the buffer
	 */
	ARCEMU_INLINE void Clear() { m_writePos = m_readPos = 0; }
