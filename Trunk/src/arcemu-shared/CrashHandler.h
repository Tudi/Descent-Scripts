...
#define STATUS_CRT_REPORT 0xE0000001
...
// Handler for CRT reports (debug assertions, etc.)
//int AscentReportHook(int nRptType, char *szMsg, int *retVal);
...
#define THREAD_HANDLE_CRASH2  } __except(HandleCrash(GetExceptionInformation())) {}
	...
	#define THREAD_HANDLE_CRASH_MapMgr  } __except(HandleCrash(GetExceptionInformation())) {}
#else
	#define THREAD_TRY_EXECUTION_MapMgr __try {
