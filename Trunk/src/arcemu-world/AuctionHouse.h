...
struct Auction
{
	...
	void UpdateInDB();
	void AddToPacket(WorldPacket & data, uint32 counter);
	bool Deleted;
...
public:
	...
	void SendPendingSalesListPacket(Player * plr, WorldPacket * packet);
	...
	void SendAuctionOutBidNotificationPacket(Auction * auct, uint64 newBidder, uint64 newHighestBid);
