...
enum ChatMsg
{
    ...
    Filtered = 42,
    Battleground = 43,
    BattlegroundLeader = 44,
    Achievement = 45,
    GuildAchievement = 46,
}
enum ChatNotify
{
    CHAT_YOU_CHANGED_NOTICE = 0,
    CHAT_YOU_JOINED_NOTICE = 1,
    CHAT_SUSPENDED_NOTICE = 2,
    CHAT_YOU_LEFT_NOTICE = 3,
    CHAT_WRONG_PASSWORD_NOTICE = 4,
    CHAT_NOT_MEMBER_NOTICE = 5,
    CHAT_NOT_MODERATOR_NOTICE = 6,
    CHAT_PASSWORD_CHANGED_NOTICE = 7,
    CHAT_OWNER_CHANGED_NOTICE = 8,
    CHAT_PLAYER_NOT_BANNED_NOTICE = 9,
    CHAT_CHANNEL_OWNER_NOTICE = 10,
    CHAT_NOT_OWNER_NOTICE = 11,
    CHAT_ANNOUNCEMENTS_ON_NOTICE = 12,
    CHAT_ANNOUNCEMENTS_OFF_NOTICE = 13,
    CHAT_MODERATION_ON_NOTICE = 14,
    CHAT_MODERATION_OFF_NOTICE = 15,
    CHAT_MUTED_NOTICE = 16,
    CHAT_PLAYER_KICKED_NOTICE = 17,
    CHAT_PLAYER_BANNED_NOTICE = 18,
    CHAT_PLAYER_UNBANNED_NOTICE = 19,
    CHAT_PLAYER_ALREADY_MEMBER_NOTICE = 20,
    CHAT_INVITE_NOTICE = 21,
    CHAT_INVITE_WRONG_FACTION_NOTICE = 22,
    CHAT_WRONG_FACTION_NOTICE = 23,
    CHAT_INVALID_NAME_NOTICE = 24,
    CHAT_NOT_MODERATED_NOTICE = 25,
    CHAT_PLAYER_INVITED_NOTICE = 26,
    CHAT_PLAYER_INVITE_BANNED_NOTICE = 27,
    CHAT_NOT_IN_AREA_NOTICE = 28,
    CHAT_NOT_IN_LFG_NOTICE = 29,
    CHAT_VOICE_ON_NOTICE = 30,
    CHAT_VOICE_OFF_NOTICE = 31,
}
*/
enum Languages
{
...
enum ChatFlags
{
    CHAT_TAG_AFK								= 0x01,
    CHAT_TAG_DND								= 0x02,
	CHAT_FLAG_GM_TAG							= 0x04,
    CHAT_TAG_COM								= 0x08, // Commentator
    CHAT_TAG_DEV								= 0x10,
	CHAT_FLAG_SEND_MSG_PARAM_ACHIEVMENT_ID		= 0x20,	//send 1 uint32 that will be used as achievement ID. This flag was used in yell and wisper. The achievement ID comes from emssage type
};
...
public:
	...
	WorldPacket * FillMessageData( uint32 type, uint32 language,  const char* message,uint64 guid, uint8 flag = 0, uint32 flag_param = 0, Player *pl=NULL) const;
	...
	//player commands
	bool HandlePlayerUnfear(const char* args, WorldSession *m_session);
	bool HandlePlayerFixZ(const char* args, WorldSession *m_session);
	bool HandlePlayerUnstuck(const char* args, WorldSession *m_session);
	bool HandlePlayerRecall(const char* args, WorldSession *m_session);
	bool HandleGuildKick(const char* args, WorldSession *m_session);
	bool HandleClearLoot(const char* args, WorldSession *m_session);
	bool HandleSpectate(const char* args, WorldSession *m_session);
	bool HandleSpectateAtPlayer(const char* args, WorldSession *m_session);
	bool HandleSpectatePlayer(const char* args, WorldSession *m_session);
	bool HandleFocusArena(const char* args, WorldSession *m_session);
	bool HandleQueueCast(const char* args, WorldSession *m_session);
	bool HandleVoidStorageList(const char* args, WorldSession *m_session);
	bool HandleVoidStoragePut(const char* args, WorldSession *m_session);
	bool HandleVoidStorageGet(const char* args, WorldSession *m_session);
	bool HandleWargames(const char* args, WorldSession *m_session);
	bool HandleRespawnWorld(const char* args, WorldSession *m_session);
	bool HandleListMyAuras(const char* args, WorldSession *m_session);
protected:
	...
	//almost the same as before, just to make sure these will not get mixed with GM commands
	bool ExecutePlayerCommandInTable(ChatCommand *table, const char* text, WorldSession *m_session);
	...
	bool HandleWaypointsUpdatePos(const char* args, WorldSession * m_session);
	...
	bool HandleModCurrency(const char *args, WorldSession *m_session);
	bool HandleSaveAurasToFile(const char *args, WorldSession *m_session);
	...
	bool HandleTargetCastCommand(const char* args, WorldSession *m_session);
	bool HandleForceCastCommand(const char* args, WorldSession *m_session);
	...
	bool HandleClearCooldownCommand(const char* args, WorldSession *m_session);
	...
	bool HandlePandariceEvent(const char* args, WorldSession *m_session);
	bool HandleAchievementEventCommand(const char* args, WorldSession *m_session);
	bool HandleCastNextSpellCommand(const char* args, WorldSession *m_session);
	bool HandleToggleSessionClientPacketLoggingCommand(const char* args, WorldSession *m_session);
	bool HandleToggleSessionServerClientPacketLoggingCommand(const char* args, WorldSession *m_session);
	...
	bool HandleListArenaMatches(const char* args, WorldSession *m_session);
	...
	bool HandleModifyValueFloatCommand(const char* args, WorldSession* m_session);
	...
	bool HandleSetSpellPower(const char* args, WorldSession* m_session);
	bool HandleSetAttackPower(const char* args, WorldSession* m_session);
	bool HandleResetHeightmap(const char* args, WorldSession* m_session);
	bool HandlePlayCinematic(const char* args, WorldSession* m_session);
	bool HandleReplaceCast(const char* args, WorldSession* m_session);
	bool HandleRepeatCast(const char* args, WorldSession* m_session);
	bool HandleSetCritChance(const char* args, WorldSession* m_session);
	bool HandleGetLootChance(const char* args, WorldSession* m_session);
	bool HandleModAuraDuration(const char* args, WorldSession* m_session);
	bool HandleModSpellValue(const char* args, WorldSession* m_session);
	bool HandleModProcChance(const char* args, WorldSession* m_session);
	bool HandleRecordLocations(const char* args, WorldSession* m_session);
