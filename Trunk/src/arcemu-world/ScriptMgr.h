...
enum ServerHookEvents
{
	...
	SERVER_HOOK_EVENT_ON_DUEL_ENDED			= 31,
	SERVER_HOOK_EVENT_ON_ADD_REM_ITEM		= 32,
	SERVER_HOOK_EVENT_ON_BG_ENDED			= 33,
...
typedef void(*tOnDuelEnded)(Player * pWinner, Player * pVictim);
...
typedef void(*tOnAddRemItem)(Player * pPlayer, Item *item, bool Add);
typedef void(*tOnBGEnded)( CBattleground *bat );
...
public:
	...
	CreatureAIScript * CreatePetAIScriptClassForEntry(Creature* pCreature);
...
//	bool CallScriptedDummyAura( uint32 uSpellId, uint32 i, Aura* pAura, bool apply);
	bool CallScriptedDummySpell(SpellEntry *sp, uint32 i, Spell* pSpell);
	bool CallScriptedDummyAura( SpellEntry *sp, uint32 i, Aura* pAura, bool apply);
	bool CallScriptedItem(Item * pItem, Player * pPlayer);

	//creatures are not taimed animals
	void register_creature_script(uint32 entry, exp_create_creature_ai callback);
	//pets have spellbar and can be controlled. Non pets can be wild summons, guardians..
	void register_pet_script(uint32 entry, exp_create_creature_ai callback);
	...
	void CreateEmtryCreatureNameAndProto( uint32 Entry );

protected:
	InstanceCreateMap mInstances; 
	CreatureCreateMap _creatures;
	CreatureCreateMap _pets;
...
//!! pets cannto be scripted atm. Need to make a new API for them
...
public:
	...
	{
		//in theory this is made server side also. But let's make double sure
		if( _unit )
		{
			//there is a frecvent mem corruption reason that when deleting scripts there are still events attached to other objects that will corrupt memory
			sEventMgr.RemoveEvents( _unit, EVENT_SCRIPT_UPDATE_EVENT );
			_unit->SetScript( NULL );
		}
		_unit = NULL;
	};
	...
	{
		//!!! added this quite late. might bug out some encounters. Please make sure to manually add after death events to avoid dead fighting minions
		RemoveAIUpdateEvent();
	}
	...
	virtual void OnLoad() {}	//post OnPushToWorld ( can cast auras )
	virtual void OnPrePushToWorld() {}	//right before the mob is spawned visually client side
	...
	virtual bool OnVendorInventoryShow(Player* pPlayer, ItemPrototype *pItemPrototype, uint32 Currency ) { return true; }
	virtual uint32 OnVendorItemSell( Player* pPlayer, ItemPrototype *pItemPrototype, uint32 Currency, uint32 Count ) { return 0x00FFFFFF; }
	...
	//in case you want to set some internal variable of a custom object. You will need to know the variable ID and script this function ...
	virtual void SetValue(int ValueName,int32 Value){};
	virtual void SetValue(int ValueName,float Value){};

protected:
	Creature* _unit;
	uint32		ScriptTypeID;	//can be used to detect/match script type for forced type casts
};
...
public:
	...
	void OnDuelEnded(Player * pPlayer, Player * pVictim);
	...
	void OnBGEnded( CBattleground *bat );
};
