...
public:
	...
	uint32 RemoveItemAmt(uint32 id, uint32 amt, bool IncBank=true);
	...
	void BuyCurrency(uint32 cur_id, uint32 total_amount, Creature * pVendor);
...
//	int16 FindFreeCurrencySlot();
	...
	uint8 CanAffordCurrency(uint32 cur_id,uint32 amount, Creature * pVendor);
   ...
   ARCEMU_INLINE bool VerifyBagSlotsWithBank(int8 ContainerSlot, int16 Slot)
   {
    ...
    Item *AddItemById(uint32 itemid, uint32 count, int32 randomprop);
	int8 CanEquipItem(ItemPrototype *proto, Player *m_owner);
	bool HasItemWithTotemCategory(uint32 category);
private:
