...
//	sEventMgr.ModifyEventTime(World::getSingletonPtr(),ref,newinterval);
	sEventMgr.ModifyEventRepeatInterval(World::getSingletonPtr(),ref,newinterval);
	RELEASE_LOCK
	return 0;
}
...
{
	char * funcName = strdup(luaL_checkstring(L,1));
...
public:
	...
	void Destroy()
	{
		...
		//but we are removing all events on destroy, is this even necesarry ?
/*		{
...
#define CHECK_INT(L, narg) (int32)luaL_checkinteger((L),(narg))

#define PUSH_UNIT(L, unit) sLuaMgr.PUSH_UNIT(static_cast<Unit*>(unit),L)
