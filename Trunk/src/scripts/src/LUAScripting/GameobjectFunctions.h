...
//		sEventMgr.ModifyEventTimeAndTimeLeft(ptr, EVENT_SCRIPT_UPDATE_EVENT, newtime);
	...
	int GetClosestPlayer(lua_State * L, GameObject * ptr)
	{
		...
		for (InRangeSet::iterator itr = ptr->GetInRangeSetBegin(); itr != ptr->GetInRangeSetEnd();)
		{
...
//		gs->o3 = go->GetParentRotation(3);
	...
	int GetInRangePlayers(lua_State * L, GameObject * ptr)
	{
		...
		uint32 count = 0;
		lua_newtable(L);
		for(InRangeSet::iterator itr = ptr->GetInRangeSetBegin(); itr != ptr->GetInRangeSetEnd(); itr++)
		{
			if( (*itr)->IsPlayer() == false )
				continue;
			if((*itr)->IsUnit())
