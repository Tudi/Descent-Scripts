...
bool ADireSituation(uint32 i, Spell* pSpell)
{
  ...
  if(qle == NULL)
	...
	if( (int32)qle->GetMobCount(0) < qle->GetQuest()->required_mobcount[0])
	{
...
bool AllAlongtheWatchtowers(uint32 i, Spell* pSpell)
{
	...
	if(tower2 != NULL)
	{
		...
		if( (int32)qle->GetMobCount(1) < qle->GetQuest()->required_mobcount[1])
		{
	...
	if(tower3 != NULL)
	{
		...
		if( (int32)qle->GetMobCount(2) < qle->GetQuest()->required_mobcount[2])
		{
	...
	if(tower4 != NULL)
	{
		...
		if( (int32)qle->GetMobCount(3) < qle->GetQuest()->required_mobcount[3])
		{
...
bool ThirstUnending(uint32 i, Spell* pSpell)
{
  ...
  {
	if(qle && (int32)qle->GetMobCount(0) < qle->GetQuest()->required_mobcount[0] )
	{
	  qle->SetMobCount(0, qle->GetMobCount(0)+1);
	  qle->SendUpdateAddKill(0);
	  qle->UpdatePlayerFields();
...
class RingOfValorFire : public GameObjectAIScript
{
public:
    RingOfValorFire(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
    static GameObjectAIScript *Create(GameObject * GO) { return new RingOfValorFire(GO); }

    void OnPushToWorld()
    {
		if( _gameobject->GetMapId() == 618 ) //ring of valor
			RegisterAIUpdateEvent( 500 );
    }
	void AIUpdate()
	{
		InRangePlayerSetRecProt::iterator itr2;
		_gameobject->AquireInrangeLock(); //make sure to release lock before exit function !
		InrangeLoopExitAutoCallback AutoLock;
		for( itr2 = _gameobject->GetInRangePlayerSetBegin( AutoLock ); itr2 != _gameobject->GetInRangePlayerSetEnd(); itr2++)
		{
			if( (*itr2)->isAlive() == false )
				continue;
			if( (*itr2)->GetPositionX() < 0.0f || (*itr2)->GetPositionX() > 0.0f)
				continue;
			if( (*itr2)->GetPositionY() < 0.0f || (*itr2)->GetPositionY() > 0.0f)
				continue;

			uint32 damage = (*itr2)->GetMaxHealth() * 5 / 100;
			WorldPacket data(SMSG_ENVIRONMENTALDAMAGELOG, 21);
			data << (*itr2)->GetGUID() << uint8(DAMAGE_LAVA) << damage << uint64(0);
			(*itr2)->SendMessageToSet(&data, true);

			(*itr2)->DealDamage( (*itr2), damage, 0);
		}
		_gameobject->ReleaseInrangeLock();
	}
};

void SetupQuestItems(ScriptMgr * mgr)
{
//	mgr->register_gameobject_script(192387, &RingOfValorFire::Create);
//	mgr->register_gameobject_script(192388, &RingOfValorFire::Create);
	mgr->register_gameobject_script(192704, &RingOfValorFire::Create);
	mgr->register_gameobject_script(192705, &RingOfValorFire::Create);
